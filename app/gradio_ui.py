...
DESTINATIONS = json.loads((BASE_DIR / "data" / "destinations.json").read_text(encoding="utf-8"))
DEST_CHOICES = list(DESTINATIONS.keys())
...
def run_prediction(concert_end_hour, day_type, concert_size, weather,
    ...
    try:
        ...
        dest_km = DESTINATIONS.get(destination_zone, 8.0)
...
| Parameter | Nilai |
|---|---|
| Tujuan | {destination_zone} |
| Jarak dari GBK | {dest_km} km |
"""

        rows = []
        for opt in result.options:
            label = opt.mode.replace("_", " ").title()
            if opt.mode == result.best_option:
                label = label + "  [TERBAIK]"
            rows.append([
                label,
                opt.pickup_point,
...
"""
        ...
        bar_colors = ["#4CAF50" if o.mode == result.best_option else "#2196F3"
    ...
    **Gelora Bung Karno, Jakarta** — Prediksi surge price ojol dan rekomendasi rute penjemputan terbaik
    ...
    with gr.Row():
        with gr.Column(scale=1, min_width=280):
            ...
            destination_zone = gr.Dropdown(
                choices=DEST_CHOICES,
                value=DEST_CHOICES[0],
                label="Tujuan Akhir",
                info="Ketik untuk mencari lokasi tujuan",
                filterable=True
            )
            btn = gr.Button(
        ...
        with gr.Column(scale=2):
            ...
            surge_out = gr.Markdown(
                value="Isi form di kiri lalu klik tombol untuk mendapatkan rekomendasi."
            )
